...
export default function Repos() {
  return (
    <section id="repos">
      <h1>Repositorios</h1>
      <p className="lead">
        AVACOM tiene dos repos activos en GitHub: un frontend activo y un backend en producción.
      </p>

      <div className="callout callout-blue">
        <strong>Repo activo:</strong> <code>website_3.0</code> es el frontend en producción.
        Todo el trabajo de migración Next.js ocurre ahí. El backend vive en <code>website_backend</code>.
      </div>
        ...
        <tbody>
          {REPOS.map(r => (
            <tr key={r.name}>
              ...
              <td>
                {r.url
                  ? <a href={r.url} target="_blank" rel="noreferrer">{r.url.replace('https://', '')}</a>
                  : '—'}
              </td>
              <td>{r.desc}</td>
              <td>{r.stack}</td>
      ...
      <p>
        Frontend y backend son repos independientes con sus propios pipelines CI/CD.
        El frontend se despliega en Vercel (o Cloudflare Pages), el backend en AWS vía SAM.
        Se comunican únicamente a través de <code>https://api-email.avacomworld.com</code>.
      </p>
    </section>
  )
}
